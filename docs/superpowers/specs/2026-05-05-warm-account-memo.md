...
- 将超时基线固化到各函数 `config.json`：`login/book/category/init-database=20` 秒、`record=10` 秒、`init-collections=3` 秒、`clear-test-data=60` 秒。（2026-07-05 内容安全修复后，`record` 调整为 20 秒。）
...
## 三十三、UI 色彩系统、云端主题同步与分类图标系统（2026-07-03）

### 背景

用户反馈当前主题色观感较差，且分类 emoji 不够统一。讨论后确认默认主题采用牛油果绿，并提供青苹果绿、牛油果绿、淡粉、白色等多主题选择；同时将分类展示从系统默认 emoji 升级为统一图标系统，保留旧 emoji 兼容。

### 实施方案

- 新增 `miniprogram/utils/theme.js`，集中维护主题列表、默认主题、语义色、统计色板、主题缓存和运行时应用逻辑。
- 主题本地缓存按 `openId` 区分；`login` 云函数新增 `updatePreferences`，将 `members.preferences.themeId` 作为云端恢复来源。
- 新增 `miniprogram/utils/category-icons.js` 和 `components/category-icon`，系统预设通过 `presetKey` 映射本地 PNG，用户自定义分类支持 `iconKey`，旧 `icon` emoji 兜底。
- 更新首页、记一笔、统计、预算、详情、分类管理、我的、家庭、登录等页面的主题变量和分类图标展示。
- tabBar 图标重新生成本地 PNG，选中态跟随主题强调色；白色主题使用石墨灰强调色。
- 统计页改为分类、记录、预算并行加载；预算接口失败时只降级预算展示，不阻塞饼图和分类占比。
- 家庭页隐藏未完成的移除成员入口；详情页删除按钮改为创建者或账本管理员可见。

### 验证重点

- 新用户默认牛油果绿；切换主题后立即生效，重启、清缓存、换设备后能从云端恢复。
- 同一设备切换账号时主题不串号。
- 白色主题下 tabBar、汇总卡、预算卡、按钮、链接清晰可读。
- 系统分类使用统一图标，旧 emoji 分类继续展示；自定义分类选择内置图标后各页面一致。
- 统计页预算失败时，饼图和分类占比仍正常展示。
- 普通非创建者不显示删除按钮；创建者和账本管理员显示删除按钮，后端仍最终校验权限。

---

## 三十四、UI 主题与图标上线审查收口（2026-07-03）

### 对抗性审查结论

本轮未发现会直接造成数据破坏或后端权限绕过的 P0 问题。自动化检查已通过：`npm test` 47 项通过，关键 JavaScript 文件通过 `node --check`，主包约 1.21 MiB，图片单文件均小于 200 KiB，`lazyCodeLoading: requiredComponents` 保持启用。

需要在上线前额外关注的风险：

- 主题系统依赖 WXSS CSS 自定义属性与运行时 tabBar API，Node 测试无法覆盖真实小程序渲染；必须在开发者工具和真机确认主题切换后页面主体、汇总卡、预算卡、tabBar 均同步变化。
- 主题云同步失败当前只记录控制台日志，本地会立即生效，但清缓存或换设备后可能无法恢复该次选择；后续应增加重试或用户可见提示。
- 详情页管理员删除按钮在审查时曾通过 `book.get` 间接判断 owner；后续记录编辑改造已改为 `record.get` 按 `recordId + bookId` 返回 `canModify`，前端编辑/删除入口统一使用该结果。
- `themeId` 与 `iconKey` 当前主要由客户端规范化，云函数侧后续应补充白名单校验，避免脏数据长期进入 `members` 或 `categories`。

### 专项发布手册

新增 `docs/superpowers/plans/2026-07-03-ui-theme-icon-release-runbook.md`。本手册只覆盖此次修改影响到的验证点：

- 默认牛油果绿、主题切换、白色主题可读性、云端主题恢复与账号隔离。
- 系统分类 PNG 图标、自定义分类内置图标、旧 emoji 兼容。
- 统计页预算失败时饼图和分类占比降级正常。
- 家庭页不显示“移除成员”；详情页创建者/管理员删除按钮显示正确。

不在本轮专项手册中重复完整 A/B 家庭合并、邀请码过期、迁移中断、预算全链路、游客模式或旧版本更新全量回归。

---

## 三十五、ignored 本地配置文件缺失复盘（2026-07-03）

### 问题现象

在 `.worktrees/warm-account/` 中完成本轮 UI 主题与图标改动后，`miniprogram/utils/env.js` 缺失。`config.js` 仍然 `require('./env')`，因此微信开发者工具编译或本地运行会因为缺少本地云环境配置而失败。

### 根因

- `miniprogram/utils/env.js` 被 `.gitignore` 忽略，属于本地敏感配置，不纳入版本控制。
- Git worktree 只检出被版本管理的文件，不会自动复制 ignored 文件；根目录或其他 worktree 中存在 `env.js`，不代表 `.worktrees/warm-account/` 中也存在。
- 普通 `git status --short` 不会显示 ignored 文件缺失，无法暴露此问题。
- Node 自动测试中对 `env.js` 做了 stub，`npm test` 通过不能证明小程序运行配置完整。

### 固化规则

- 已在仓库根目录 `AGENTS.md` 中新增硬规则：每次新建/切换 worktree、预览、发布或修改 `config.js` 相关逻辑前，必须显式检查 `.worktrees/warm-account/miniprogram/utils/env.js` 是否存在并可被 `config.js` 加载。
- 必查命令：`Test-Path miniprogram/utils/env.js`、`git status --ignored miniprogram/utils/env.js`、`node -e "require('./miniprogram/utils/config'); console.log('env ok')"`。
- 如果 `env.js` 缺失，先从本机已有副本恢复，或复制 `env.example.js` 并填入真实 `env/appId`。
- 以后不得用普通 `git status` 或 `npm test` 代替 ignored 本地配置检查。

---

## 三十六、分类语义图标与主题弹层滚动修复（2026-07-04）

### 背景

测试过程中发现“预置 PNG、用户新增分类、历史 emoji”三套图标语言混在一起，首页记录虽然能显示，但新增“废品回收、报销、奖金、红包礼金”等非预置名称时，如果图标图片本身带分类文字，会出现图标语义和用户分类名不一致的问题。主题颜色底部弹层在小屏上也存在列表无法继续下滑的风险。

### 处理方案

- 将图标资产改为语义符号库：文件名使用 `icon_*`，图片内部不放中文分类名，只画碗筷、票据、奖章、红包、回收、钱包退款等可复用符号。
- `category.iconKey` 继续作为主字段，新写入只保存语义 key；`category.icon` emoji 保留为历史兜底，不再作为新增大类主入口。
- 新增 `ICON_ALIASES` 兼容旧 `expense_*` / `income_*` key，旧账本无需批量迁移即可显示新图标。
- 新增 `EMOJI_ICON_ALIASES`，将明确可识别的历史 emoji 转为现有语义 PNG；无法判断语义的 emoji 继续兜底显示，等待生产只读审计后再决定是否补新图标。
- 新增分类推荐按当前收支类型内匹配：例如支出“买菜/话费/医疗”分别推荐买菜袋、话费、医疗图标；收入“退款/报销/奖金/废品回收”分别推荐退款、票据返款、奖章、回收收益图标；当前类型无强匹配时使用默认图标。
- 分类选择器和分类管理页移除 emoji 输入入口，改为输入名称后自动推荐并可在同类型语义图库中手动选择。
- 主题弹层改为纵向 flex，标题固定，主题列表使用 `scroll-view scroll-y`，高度限制为 `max-height: 78vh` 并预留安全区。

### 验证

- `npm test` 共 50 项通过，新增覆盖语义 alias、推荐规则和同类型图库过滤。
- 关键 JS 文件通过 `node --check`。
- 新增 34 个 `icon_*.png` 透明图标，最大文件 737 bytes，所有图片均小于 200KB。
- 主包粗算约 1.233MB，`lazyCodeLoading: requiredComponents` 保持启用。
- `miniprogram/utils/env.js` 存在且 `config.js` 可正常加载。

### 生产只读审计补充

- 登录 CloudBase CLI 后只读查询 `categories_prod` 的大类，未执行写入、更新或删除操作。
- 当前生产大类主要是系统预置分类，另有 2 个自定义大类：“医保报销”（收入，历史 emoji 为 💊）和“汽车”（支出，历史 emoji 为 🚗）。
- “医保报销”不应因为历史 emoji 是药丸而显示为医疗支出图标，已改为有明确 `type` 时先按分类名语义推荐，命中 `icon_receipt_return`；“汽车”命中 `icon_vehicle`。
- 补充历史 emoji 映射：`🚉/🚆/🚄/🚅` 归一到交通，`➕/📦` 归一到默认图标。
- 结论：生产数据没有暴露语义库缺失的新增业务概念，本轮无需新增额外 PNG。

---

## 三十七、支出/收入记录编辑能力（2026-07-04）

### 背景

测试和需求讨论中确认：当前记录一旦输错，只能删除后重新记一笔。这个对真实家庭记账很不友好，尤其是带图片、跨月份、分类填错或备注需要补充时，删除重记会破坏用户对“这条账”的连续感。

### 处理方案

- 记录详情页新增“编辑记录”按钮，与“删除记录”共用后端权限口径：记录创建人和账本所有者可编辑，普通成员不能编辑他人记录。
- 记一笔页新增编辑态 `mode=edit&recordId=...`，复用原表单初始化金额、类型、分类、日期、备注和图片，保存按钮改为“保存修改”。
- `record` 云函数新增 `get` action，按 `recordId + bookId` 读取单条记录并返回 `canModify`；详情页不再只依赖首页缓存，缓存先展示后再以云端数据刷新。
- `record.update` 加固校验：账本成员和修改权限、合并态阻止、金额正整数、类型枚举、分类属于当前账本且类型匹配、日期有效、图片为字符串数组。
- 编辑保存只更新业务字段和 `updatedBy/updatedByName/updatedAt`，保留创建人字段；编辑不触发新增时的预算提醒浮层。
- 图片编辑区分预览 URL 和真实 cloud file ID：已有图片继续保留 file ID，新增本地图片才上传，删除旧图片仅从记录引用中移除。

### 验证重点

- 修改金额、日期、分类、备注后，详情页返回即刷新；首页、统计和预算页返回后按最新记录重新加载。
- 支出/收入类型切换后分类被清空，必须重新选择当前类型分类；后端也会拒绝类型不匹配分类。
- 保留旧图片并新增图片、删除旧图片后保存均能正确展示；图片上传失败时不更新记录。
- 普通成员不能编辑他人记录，账本所有者可以编辑家庭成员记录；迁移/合并中的账本不能编辑。

---

## 三十八、测试反馈修复与分类排序（2026-07-04）

### 背景

测试中发现：记一笔未选分类时会显示默认 emoji，容易误以为是分类图标；预算页大类预算行点击删除没有二次确认；首页切回时预算卡比记录慢且会闪成空状态；分类管理页缺少大类/小类顺序整理能力。

### 处理方案

- 记一笔页只在 `selectedCategory.categoryId` 存在时渲染 `category-icon`，未选择分类时只显示“请选择”。
- 大类预算行操作改名为“移除”，二次确认后仅修改本地预算草稿，并提示保存预算后生效。
- 首页预算卡增加内存缓存：按 `isTest + bookId + month` 缓存预算展示数据；切回首页时先显示旧值，后台刷新后更新。
- 记录新增/编辑/删除和预算保存/复制/删除后清理预算缓存，避免预算卡长期展示旧数据。
- `category` 云函数新增 `reorderBig` 与 `reorderChildren`，严格校验 ID 完整覆盖当前层级；新增大类/小类默认追加到同级末尾。
- 分类管理页提供上移/下移按钮；排序为账本级，所有家庭成员共享。小类只允许在原大类内部排序，不支持跨大类迁移。

### 验证重点

- 新增/编辑记账未选分类不出现默认 emoji。
- 大类预算移除取消不改变表单，确认后仍需保存预算才生效。
- 首页预算卡切回时不闪成未设置，预算接口失败不影响记录列表。
- 大类排序和小类排序刷新后保持；后端拒绝缺失、重复、跨账本或跨父类的排序请求。

---

## 三十九、品牌头像与介绍文案更新（2026-07-04）

### 背景

微信小程序后台名称一年只有两次修改机会，当前仅剩一次，且不确定更名后是否需要重新备案。因此本轮不修改小程序后台名称，也不调整 `app.json`、导航栏标题或应用内“暖账”展示，只更新头像资产和介绍表达。

### 处理方案

- 使用内置生图工具生成新的温暖 3D 风格头像：账本、爱心、房屋轮廓、金币和牛油果绿点缀；不包含文字、数字、政治敏感或成人内容。
- `miniprogram/images/logo.png` 替换为 512x512 PNG，继续供登录页和“关于”页共用，保持应用内头像一致。
- 新增 `docs/superpowers/assets/warm-account-avatar-144.png`，尺寸 144x144，作为微信后台上传头像资产。
- 用户协议中的服务内容介绍更新为：面向小家庭的协作记账，支持共同记录收支、管理分类与预算、查看消费结构，并通过邀请码共享账本。

### 验证

- `logo.png` 为 512x512、约 92KB；后台头像为 144x144、约 9.5KB。
- 两张 PNG 均低于大小限制，且小尺寸下仍能识别账本、爱心、家庭和金币元素。
- 本轮未修改小程序后台名称、页面标题、云函数、数据结构或导航配置。

---

## 四十、内容安全审核打回修复（2026-07-05）

### 背景

2026-07-04 23:04:58 提交腾讯审核后被打回，原因是【记一笔】功能在内容安全验证时存在信息安全风险，要求接入内容安全 API，并且违规提示只需要告知用户所发布内容含违规信息。

### 处理方案

- `record.add` / `record.update` 在写入记录前校验最终备注和图片；编辑态使用更新后的最终值，避免只改金额时绕过旧备注/旧图片检测。
- 备注和分类名称使用 `cloud.openapi.security.msgSecCheck`，参数包含 `openid`、`scene = 2`、`version = 2` 和待检测文本。
- 记录图片使用 `cloud.downloadFile` 读取 `cloud://` 文件内容，再调用 `cloud.openapi.security.imgSecCheck` 同步阻断；非 `cloud://` 图片或下载失败均视为检测暂不可用。
- `category.addBig` / `category.addChild` / `category.rename` 在写入前校验用户输入的分类名称，因此记一笔分类弹层和分类管理页共用后端拦截。
- 检测命中风险返回 `CONTENT_SECURITY_REJECTED` 和“内容含违规信息，请修改后再试”；接口异常、超时或 SDK 方法不可用返回 `CONTENT_SECURITY_UNAVAILABLE` 和“内容安全检测暂不可用，请稍后再试”。
- 前端不展示 label、规则、traceId 等检测细节；云函数日志只记录 action、scope、errCode、suggest、traceId，不记录用户原文或图片内容。
- 记一笔选择图片时优先使用压缩图；如果图片已上传但记录保存失败，前端 best-effort 删除本次新上传且未入库的图片，不删除编辑记录已有图片。
- `record/config.json` 超时从 10 秒调整为 20 秒，为多图同步检测预留时间。

### 验证重点

- 普通备注、无备注、普通图片记录仍可保存。
- 违规备注、违规分类名称或风险图片无法保存，前端只显示通用违规提示。
- 内容安全接口异常或超时时，记录和分类均暂不保存，并提示稍后再试。
- 保存失败后首页、详情、统计和预算没有出现半写入脏数据，新上传图片被清理。
